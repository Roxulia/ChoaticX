...
class ZoneHandlingService():
    ...
    async def get_latest_zones(self,lookback='1 years',initial_state = False):
        ...
        if initial_state:
            athHandler = ATH.ATHHandler(self.symbol,self.based_candles)
    ...
    async def update_ATHzone(self,candle):
        try:
            ...
            Ath = await ATH.ATHHandler(self.symbol).getATHFromStorage()
            if Ath['zone_high'] < candle['high']:
                ...
                athHandler = ATH.ATHHandler(symbol=self.symbol,candles=candle_data)
    ...
    async def getUpdatedATH(self):
        athHandler = ATH.ATHHandler(self.symbol)
